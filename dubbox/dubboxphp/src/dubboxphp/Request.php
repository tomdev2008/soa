...
class Request
{
	...
	private function _GuzzleGet(){
		$baseUri = substr($this->uri,0,strrpos($this->uri, "/")+1);
		$requstUri = substr($this->uri,strrpos($this->uri, "/")+1);
		$client = new \GuzzleHttp\Client(['base_uri' => $baseUri]);
        // Send a request to https://foo.com/api/test
        $response = $client->request('GET', $requstUri);
        $body = $response->getBody();
        $stringBody = (string) $body;
        $data = json_decode($stringBody,true);
        #return $this->resultArray ? $data : json_decode(json_encode($data), FALSE);
        return $this->resultArray ? $data : (Object) $data;
	}
     ...
     * @return Request
     */
    public static function post2($uri, $payload = null, $mime = null)
    {
     ...
     * @return Request
     */
    public static function gettest($uri, $mime = null)
    {
    ...
    public function resultFormat($array=true){
